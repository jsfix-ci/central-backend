    ...
    transform(row, _, done) {
      ...
      submissionToOData(fields, table, row, options).then((data) => {
        ...
        for (const field of data) {
          ...
          // if $select is there then don't return parent IDs, currently we don't support selecting parent IDs
          const fieldRefined = options.metadata ? without([parentIdProperty], field) : field;
...
const singleRowToOData = (fields, row, domain, originalUrl, query) => {
  ...
  return submissionToOData(fields, table, row, options).then((subrows) => {
    ...
    // if $select is there then don't return parent IDs, currently we don't support selecting parent IDs
    const paredRefined = options.metadata ? pared.map(p => without([filterField], p)) : pared;
